...
export type ProfileChartInput = {
  date: string; time: string; calendar: "GREGORIAN" | "JULIAN"; placeName: string;
...
function toProfile(raw: { id: number; label: string; birthDate: string; birthTime: string; calendar: "GREGORIAN" | "JULIAN"; placeName: string; latitude: string; longitude: string; timezone: string; timeZoneId: string | null; ayanamsa: "LAHIRI" | "RAMAN" | "KP" | "TRUE_PUSHYA"; divisionalFactor: number; notes: string | null }): StoredProfile {
  return { id: raw.id, label: raw.label, date: raw.birthDate, time: raw.birthTime, calendar: raw.calendar, placeName: raw.placeName, latitude: Number(raw.latitude), longitude: Number(raw.longitude), timezone: Number(raw.timezone), timeZoneId: raw.timeZoneId ?? undefined, ayanamsa: raw.ayanamsa, divisionalFactor: raw.divisionalFactor, notes: raw.notes };
}
