...
Python 星历与 PDF 生成器现在均具有 25 秒硬超时、受限 stdout 缓冲区、一次性完成保护和服务器端截断诊断日志。若发生超时、异常输出、进程启动失败、非零退出、协议格式异常或领域输入拒绝，服务端只向页面返回明确的中文安全信息，而不暴露 Python 路径、堆栈或 stderr 内容。PDF 响应额外验证文件名、base64 类型和 PDF 魔数前缀。

出生档案 CRUD 现对 DNS、超时、连接重置等瞬态数据库故障自动重建客户端并重试一次；schema 缺列仍优先走已有的 gender 兼容路径。前端增加档案列表“重新加载”操作、档案 CRUD 错误提示、PDF 失败后的原配置重试按钮和可关闭的合盘失败提示；原有 pending 禁用继续防止重复提交。浏览器已复测真实 Chennai 计算和 PDF 导出成功恢复。自动化最终结果为 `pnpm test` 共 11 个测试文件、21 个用例通过，且生产构建通过。

后续加固将同一瞬态错误策略扩展至 OAuth 用户写入、用户查询、本地账号创建与登录时间更新，避免网络波动被误显示为账号不存在或密码错误。工作台在计算失败时保留经过规范化的最后一次输入，并提供“重试”按钮；PDF、档案与合盘分别提供不丢失现有数据的恢复提示。`processErrors.test.ts` 现直接执行计算响应解析和 PDF 响应解析的格式损坏、领域拒绝、无效 PDF 载荷路径；`db.genderFallback.test.ts` 覆盖 DNS、超时与连接重置分类。最终 `pnpm test` 为 11 个测试文件、23 个用例全部通过，且生产构建成功。

## 2026-08-12：PDF 网络失败安全提示与恢复验证

在已成功生成 Chennai 校验样例真实星盘的浏览器会话中，临时将页面网络请求替换为抛出 `simulated offline` 的受控失败。点击“下载 PDF 报告”后，工作台没有显示原始英文异常；而是显示中文安全提示“PDF 导出未完成，请检查网络连接后重试。”，并保留“重试”按钮。该结果同时验证了 PDF 设置、计算结果和下载入口在错误状态下均未丢失。

随后恢复原始网络实现并点击“重试”。按钮先进入“生成 PDF…”禁用状态，服务器端重新计算和生成完成后恢复为“下载 PDF 报告”，且错误提示及重试按钮消失。由此确认 PDF 的失败提示、中文安全转换、原配置重试和正常恢复链路均在浏览器中实际运行。前端纯函数回归 `server/safeError.test.ts` 另覆盖“保留中文安全文案、替换英文传输错误、处理非 Error 值”三个分支。

## 2026-08-12：已登录档案失败恢复验证
